class Solution(object):
    def numIslands2(self, m, n, positions):
        ...
        gridcopy = [[[i,j] in positions for i in range(n)] for j in range(m)]

        p = [i for i in range(n*m)]
        self.printp(p, n)
        def find(a):
            if p[a] != a:
                # print('parent of %s is %s' % (a, p[a]) )
                p[a] = find(p[a])
                self.printp(p,n)
            return p[a]
        ...
        def getnbrs(pos, grid):
            nbrs = []
            parts = []
            vectors = [[1,0],[0,1],[-1,0],[0,-1]]
            for v in vectors:
                if 0 <= pos[0]+v[0] < len(grid) and 0 <= pos[1]+v[1] < n and grid[pos[0]+v[0]][pos[1]+v[1]] == 1:
                    x = pos[0]+v[0]
                    y = pos[1]+v[1]
                    nbrs.append(x*n + y)
            ...
            if len(nbrs) != 0:
                print('nbrs %s' % (nbrs))
                for b in nbrs:
                    parts = [p[i] for i in nbrs]
                    print('parts is %s' % (parts))
                    if find(b) != p[pos[0]*n + pos[1]]:
                        union(pos[0]*n + pos[1], b)       
            return len(set(parts))
                
        for pos in positions:
            print('pos is %s' % (pos[0] * n + pos[1]))
            grid[pos[0]][pos[1]] = 1
            xyz = getnbrs(pos,grid)
            cnt = cnt + 1 - xyz
            res.append(cnt)
        return res

    def printp(self, p, col):
        ...
        for i in range(len(m)):
            for j in range(len(m[0])):
                if m[i][j] == '*':
                    ...
                    print('%d' % (m[i][j])),
            print('\b')
    def finderror(self,m,n,pre):

        gridcopy = [[0 for i in range(n)] for j in range(m)]
        for p in pre:
            gridcopy[p[0]][p[1]] = 1
        gridcopy[9][38] = '*'
        self.printm(gridcopy)
n = 3
m = 3
...
# print(len(pre))
# print(Solution().finderror(m,n,pre))